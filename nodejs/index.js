...
const UUID = process.env.UUID || 'd2ad9807-3f00-43ab-89ce-270d9dab3055';  // 在不同的平台运行了v1哪吒请修改UUID,否则会覆盖
...
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'hidencloudfr.kils.pp.ua';           // argo固定隧道域名,留空即使用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiYTBjN2ExMTEwZjIwZTgxMzMwYTAxN2VjOGQ5MjNmYzQiLCJ0IjoiYTJhNTUzZjMtYTc0Ny00YmQ4LTlkYTYtNWFkNWIyMWJlZDdmIiwicyI6Ik56azJOVGN3WkRFdE5EWXdaaTAwTXpGaExUbGpNR0V0T0dSaE5qUXpNVGxtTW1NMyJ9';               // argo固定隧道token或json,留空即使用临时隧道
const ARGO_PORT = process.env.ARGO_PORT || 8001;             // argo固定隧道端口,使用token需在cloudflare控制台设置和这里一致，否则节点不通
const S5_PORT = process.env.S5_PORT || '24688';                   // socks5端口，支持多端口的可以填写，否则留空
const TUIC_PORT = process.env.TUIC_PORT || '';               // tuic端口，支持多端口的可以填写，否则留空
const HY2_PORT = process.env.HY2_PORT || '24688';                 // hy2端口，支持多端口的可以填写，否则留空
