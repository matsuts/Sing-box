...
const UUID = process.env.UUID || 'c531ada8-c6b7-422b-9930-4d202572fd74';  // 在不同的平台运行了v1哪吒请修改UUID,否则会覆盖
const NEZHA_SERVER = process.env.NEZHA_SERVER || 'kils.us.ci';         // 哪吒面板地址,v1形式：nz.serv00.net:8008  v0形式：nz.serv00.net
const NEZHA_PORT = process.env.NEZHA_PORT || '8083';             // v1哪吒请留空，v0 agent端口，当端口为{443,8443,2087,2083,2053,2096}时，自动开启tls
const NEZHA_KEY = process.env.NEZHA_KEY || 'l6lN3zQNRVmrwqRg9Gb0u7';               // v1的NZ_CLIENT_SECRET或v0 agwnt密钥 
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'hidencloudin.kils.pp.ua';           // argo固定隧道域名,留空即使用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiYTBjN2ExMTEwZjIwZTgxMzMwYTAxN2VjOGQ5MjNmYzQiLCJ0IjoiN2Y3MjFiNTYtMmQxNy00ZWFlLTlmMDItNzViZjQ5OTQwNDEyIiwicyI6Ik5EaGpNV1ZqTWpZdFpUaGhaQzAwTXpOa0xXSTNaRFV0TW1VNU1qRXdNVGczTldabCJ9';               // argo固定隧道token或json,留空即使用临时隧道
const ARGO_PORT = process.env.ARGO_PORT || 8001;             // argo固定隧道端口,使用token需在cloudflare控制台设置和这里一致，否则节点不通
const S5_PORT = process.env.S5_PORT || '24693';                   // socks5端口，支持多端口的可以填写，否则留空
const TUIC_PORT = process.env.TUIC_PORT || '';               // tuic端口，支持多端口的可以填写，否则留空
const HY2_PORT = process.env.HY2_PORT || '24693';                 // hy2端口，支持多端口的可以填写，否则留空
...
const BOT_TOKEN = process.env.BOT_TOKEN || '7069903272:AAEhfkTBX2Y-n3r8BMcYGHGgyue7rkxAAko';               // Telegram bot_token 两个变量不全不推送节点到TG 
